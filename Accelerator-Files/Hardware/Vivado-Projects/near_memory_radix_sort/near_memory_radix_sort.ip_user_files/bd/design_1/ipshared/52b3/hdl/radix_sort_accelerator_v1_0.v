...
module radix_sort_accelerator_v1_0 
	...
	wire [2:0] w_sort_state;
	wire [2:0] w_first_state;
	wire [3:0] w_even_state;
	wire [3:0] w_odd_state;
	wire [3:0] w_data_state;
	wire w_sort_start;
	wire w_sort_start_delay;
	wire [31:0] w_li_count;
	wire [31:0] w_rs_count;
	wire [31:0] w_so_count;
	wire [31:0] w_all_count;
	wire [9:0] w_pointer0;
	wire [9:0] w_pointer1;
	wire [9:0] w_addr1;
	wire [9:0] w_addr2;
	...
	radix_sort_accelerator_v1_0_S00_AXI_FROM_PS #(
		...
		.I_SORT_STATE(w_sort_state),
		.I_FIRST_STATE(w_first_state),
		.I_EVEN_STATE(w_even_state),
		.I_ODD_STATE(w_odd_state),
		.I_DATA_STATE(w_data_state),
		.I_SORT_START(w_sort_start),
		.I_SORT_START_DELAY(w_sort_start_delay),
		.I_POINTER0(w_pointer0),
		.I_POINTER1(w_pointer1),
		.I_ADDR1(w_addr1),
		.I_ADDR2(w_addr2),
		.I_LI_COUNT(w_li_count),
		.I_RS_COUNT(w_rs_count),
		.I_SO_COUNT(w_so_count),
		.I_ALL_COUNT(w_all_count),
	...
	(
		...
		.O_SORT_STATE(w_sort_state),
		.O_FIRST_STATE(w_first_state),
        .O_EVEN_STATE(w_even_state),
        .O_ODD_STATE(w_odd_state),
        .O_DATA_STATE(w_data_state),
		.O_SORT_START(w_sort_start),
		.O_LI_COUNT(w_li_count),
		.O_RS_COUNT(w_rs_count),
		.O_SO_COUNT(w_so_count),
		.O_ALL_COUNT(w_all_count),
		.O_POINTER0(w_pointer0),
		.O_POINTER1(w_pointer1),
		.O_ADDR1(w_addr1),
		.O_ADDR2(w_addr2),
		.O_SORT_START_DELAY(w_sort_start_delay)
	);
